...
def handle_stage_clear_screen(screen, ui_font, title_font, cursor_pos, score, current_stage, stage_passed, stage_targets, hover_progress):
    ...
    label = "STAGE CLEAR!" if stage_passed else "TIME'S UP!"
    ...
    if stage_passed:
        result_txt = ui_font.render(f"TARGET MET — {score} / {target} GHOSTS CAPTURED", True, (0, 255, 150))
    else:
        result_txt = ui_font.render(f"TARGET MISSED — {score} / {target} GHOSTS CAPTURED", True, (255, 100, 100))
    screen.blit(result_txt, (WIDTH // 2 - result_txt.get_width() // 2, 480))

    # Next stage preview
    next_stage = current_stage + 1
    next_labels = {2: "STAGE 2 — FASTER GHOSTS", 3: "STAGE 3 — DANGER MODE (DECOYS ACTIVE)"}
...
def handle_tutorial_rendering(screen, ui_font, title_font, countdown_font, phase, tutorial_count, now, ready_timer, mp_tutorial_count=0):
    ...
    # PHASE_TUTORIAL_MP (multiplayer training)
        ...
        t1 = ui_font.render("P1 — USE YOUR GREEN OBJECT  |  P2 — USE YOUR PURPLE OBJECT", True, (255, 230, 150))
        t2 = ui_font.render("MOVE YOUR CROSSHAIR OVER THE GHOST TO REGISTER A HIT.", True, (200, 200, 255))
    ...
    # PHASE_INSTRUCT Layout
        ...
        screen.blit(i1, (WIDTH // 2 - i1.get_width() // 2, 200))
        screen.blit(i2, (WIDTH // 2 - i2.get_width() // 2, 350))
        screen.blit(i3, (WIDTH // 2 - i3.get_width() // 2, 410))
        screen.blit(i4, (WIDTH // 2 - i4.get_width() // 2, 470))
        screen.blit(i6, (WIDTH // 2 - i6.get_width() // 2, 620))
