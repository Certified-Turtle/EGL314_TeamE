...
def _fire_spotlights():
    ...
    Dimmer is sent FIRST so the fixture is active before attributes are set.
    """
    # Spotlight A — Fixture 502
    _set_dimmer(SPOTLIGHT_A, SPOTLIGHT_DIMMER)          # Turn on first
    _set_colour(SPOTLIGHT_A, 255, 255, 255)             # Pure white
    ...
    # Spotlight B — Fixture 202
    _set_dimmer(SPOTLIGHT_B, SPOTLIGHT_DIMMER)          # Turn on first
    _set_colour(SPOTLIGHT_B, 255, 255, 255)             # Pure white
    ...
    print(f"[LIGHTING] Spotlights ON — {SPOTLIGHT_A} pan={SPOTLIGHT_A_PAN} tilt={SPOTLIGHT_A_TILT}")
    print(f"[LIGHTING] Spotlights ON — {SPOTLIGHT_B} pan={SPOTLIGHT_B_PAN} tilt={SPOTLIGHT_B_TILT}")
...
def _setup_thumbsup():
    """Pure white everywhere for gesture reading. No pan/tilt. Spotlights stay."""
    for fix in ALL_EPAR:
