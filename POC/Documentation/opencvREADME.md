# Opencv.py
 This is the code responsible for the hand gesture tracking as well as the green colour tracking used in the game. Allowing the webcam to recognise specific hand gestures from a library.

 The code starts with these imports:
...
This section imports all required libraries used for computer vision, gesture recognition, data processing, and game integration.

This file uses OpenCV for image processing, MediaPipe for tracking, NumPy for numerical computations, and Pygame for integration with the game environment. Additional Python libraries are used for CSV data handling and gesture-related calculations.
...
These two values define the color range for detecting green objects in HSV color space.


GREEN_LOWER sets the minimum HSV values for what counts as green, the lower thresholds help detect darker or less saturated shades of green which is helpful in darker enviroments, convorsely, GREEN UPPER stes the maximum HSV value for what counts as green expanding the range to lighter shades such as neomn green.

This is important as we are using green to control the cursor on screen for gameplay.
...
This part of the code initialises the Mediapipe hand model, which is responsible for detecting and tracking hand landmarks, (fingers, joints, and wrist) from the webcam feed.

static_image_mode enables real time video, max_num_hands sets the limit to how many hands can be detected, model_complexity sets it to the lightweight model which prioritises speed over accuracy, min_detection_confidence sets minimum confidence to detect a hand and min_tracking_confidence is the minimum confidence needed to track a hand.
...
imports OpenCV which is used in accessing and controling the webcam.
...
This is the code responsible for stariing and configuring the webcam so that the program can use it for real time video input.
...
This is the code responsible for loadiing and prepareing the stored hand gestures from the csv file we created with all the gestures data, to be used for gesture recognition.
...
each detect either a thumbs up sign or ok sign respectively. Doing so by comparing the live hand to stored csv data or using basic finger position rules.
...
This part of the code processes each frame of the webcam and extracts a cleaned video frame for display, a colour based track cursor, optional hand landmarks and a warning flag for invalid input tracking.

The following portion of the code taken from the above:
```bash
# === 3. SELECTIVE SKELETAL TRACKING (ONLY RUNS ON DEMAND) ===
    hand_landmarks_list = None
    wrist_pixel_pos = None
...
This is an optional function that when enabled, runs Mediapipe on the processed frame, detecting hand landmarks and full hand landmark data and wrist positioning.
