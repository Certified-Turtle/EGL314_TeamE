...
def render_game_over_screen(screen, score):
    ...
    audio.win_pt() if score >= 10 else audio.lose_pt()
    
    f1, f2 = pygame.font.SysFont("Courier New", 150, bold=True), pygame.font.SysFont("Courier New", 60, bold=True)
    title_str = "CLEARED!" if score >= 10 else "GAME!"
    title_surf = f1.render(title_str, True, (200, 255, 220) if score >= 10 else (220, 40, 60))
    score_surf = f2.render(f"PHANTOMS SWEPT: {score} (Target: 10)", True, (255, 215, 0))
