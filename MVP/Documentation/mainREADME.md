...
When main.py is running, the game session is created

---
Starting by importing the following libraries:
...
| `import time` | This imports the time function that is used within main.py to set |

After, the following are imported:
...
By having them written elsewhere and imported in, it helps reduce the lengh of main.py and makes adjustments to the game code easier without breaking the whole game logic if a mistake is made.
...
| `import lighting` and `import audio` | Takes the commands and translates them into physical actions like flashing the room lights or playing sound effects on a different laptop. |
---
...
All gameplay is rendered in onto a fixed 1920 X 1080 virtual screen before being smoothly rescaled to the native monitor resolution. This guarantees the game UI elements look identical on any display layout without breaking aspect ratios.
...
Explaining this section portion by portion:
```bash
# --- 3. MAIN RUNTIME LOOP ---
while True:
    now = pygame.time.get_ticks()
...
This adds a bunch of keys, both for the purpose of testing.

- Pressing S converts the live camera frame buffer from RGB to BGR and writes a time-stamped snapshot to a /calibration_snapshots directory for rapid HSV color tuning
- Pressing C toggles a camera feed that allows us to see what the camera is tracking for the purpose of testing.
- Pressing P pauses the game
- Pressing the left or right arrow advances or goes back a gamestage accordingly which helps speed up testing of different game stages.
...
This line of code contains all the design elements of the game such as warning screens, background etc.
